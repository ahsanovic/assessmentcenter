            ...
            <div class="card">
                <div class="card-body">
                    <x-monitoring.page-header
                        icon="bar-chart-2"
                        color="success"
                        title="Hasil Tes PSPK"
                        description="Lihat dan unduh hasil ujian PSPK peserta per event."
                    />
                            ...
                            <tbody>
                                @foreach ($data as $index => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-xs btn-warning {{ $item->hasil_pspk_count == 0 ? 'disabled' : '' }}" wire:navigate
                                                href="{{ route('admin.tes-selesai.pspk.show-peserta', ['idEvent' => $item->id]) }}">
                                                    {{ $item->hasil_pspk_count ?? 0 }} orang
                                            </a>
                                        </td>
                                    </tr>
                                @endforeach
