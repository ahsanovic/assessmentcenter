...
class ShowPesertaSubTes2 extends Component
{
    ...
    public function render()
    {
        $data = Peserta::join('hasil_intelektual', 'hasil_intelektual.peserta_id', '=', 'peserta.id')
            ->join('ujian_intelektual_subtes_2', 'ujian_intelektual_subtes_2.peserta_id', '=', 'peserta.id')
            ...
            ->select(
                ...
                'ujian_intelektual_subtes_2.created_at as waktu_mulai',
                'ujian_intelektual_subtes_2.id as ujian_intelektual_subtes_2_id',
                'is_finished'
            )
        ...
        return view('livewire.admin.data-tes.tes-intelektual.tes-selesai.show-peserta-subtes-2', [
            'data' => $data
        ]);
    }
    ...
    {
        try {
            $updated = UjianIntelektualSubTes2::where('event_id', $this->id_event)
    ...
    {
        try {
            $hasil_subtes_2 = HasilIntelektual::findOrFail($this->selected_id);

            UjianIntelektualSubTes2::where('peserta_id', $hasil_subtes_2->peserta_id)
                ->where('event_id', $hasil_subtes_2->event_id)
                ->delete();

            $hasil_subtes_2->update([
                'nilai_subtes_2' => null
            ]);
