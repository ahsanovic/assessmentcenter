...
class ShowPesertaSubTes1 extends Component
{
    use WithPagination;
    ...
    public function mount($idEvent)
    {
        ...
        $this->event = Event::with(['pesertaTesIntelektualSubTes1'])->findOrFail($this->id_event);
    }

    public function render()
    {
        $data = Peserta::join('hasil_intelektual', 'hasil_intelektual.peserta_id', '=', 'peserta.id')
            ->join('ujian_intelektual_subtes_1', 'ujian_intelektual_subtes_1.peserta_id', '=', 'peserta.id')
            ...
            ->select(
                ...
                'ujian_intelektual_subtes_1.created_at as waktu_mulai',
                'ujian_intelektual_subtes_1.id as ujian_intelektual_subtes_1_id',
                'is_finished'
            )
        ...
        return view('livewire.admin.data-tes.tes-intelektual.tes-selesai.show-peserta-subtes-1', [
            'data' => $data
        ]);
    }
    ...
    {
        try {
            $updated = UjianIntelektualSubTes1::where('event_id', $this->id_event)
    ...
    {
        try {
            $hasil_subtes_1 = HasilIntelektual::findOrFail($this->selected_id);

            UjianIntelektualSubTes1::where('peserta_id', $hasil_subtes_1->peserta_id)
                ->where('event_id', $hasil_subtes_1->event_id)
                ->delete();

            $hasil_subtes_1->update([
                'nilai_subtes_1' => null
            ]);
