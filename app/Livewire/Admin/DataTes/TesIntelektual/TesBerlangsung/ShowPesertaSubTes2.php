...
class ShowPesertaSubTes2 extends Component
{
    ...
    public function render()
    {
        $data = Peserta::join('ujian_intelektual_subtes_2', 'ujian_intelektual_subtes_2.peserta_id', '=', 'peserta.id')
            ...
            ->select('peserta.*', 'soal_id', 'jawaban', 'ujian_intelektual_subtes_2.is_finished', 'ujian_intelektual_subtes_2.id as ujian_intelektual_subtes_2_id', 'ujian_intelektual_subtes_2.created_at as mulai_tes')
    ...
    {
        $this->validate([
            'waktu' => 'required|numeric|min:1|max:8',
        ], [
            'waktu.required' => 'Waktu tes harus diisi',
            'waktu.numeric' => 'Waktu tes harus berupa angka',
            'waktu.min' => 'Tambahan waktu tes intelektual subtes 2 minimal 1 menit',
            'waktu.max' => 'Tambahan waktu tes intelektual subtes 2 maksimal 8 menit',
        ]);

        try {
            $ujian = UjianIntelektualSubTes2::find($this->selected_id);
            if (!$ujian) {
    ...
    {
        $this->validate([
            'waktuMassal' => 'required|numeric|min:1|max:8',
        ], [
            'waktuMassal.required' => 'Waktu tes harus diisi',
            'waktuMassal.numeric' => 'Waktu tes harus berupa angka',
            'waktuMassal.min' => 'Tambahan waktu tes intelektual subtes 2 minimal 1 menit',
            'waktuMassal.max' => 'Tambahan waktu tes intelektual subtes 2 maksimal 8 menit',
        ]);

        $menit = (int) $this->waktuMassal;

        try {
            $ujians = UjianIntelektualSubTes2::where('event_id', $this->id_event)
