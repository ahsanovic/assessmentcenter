<?php

namespace App\Livewire\Admin\DataTes\TesKompetensiTeknis\TesBerlangsung;

use App\Models\Event;
use Livewire\Attributes\Layout;
use Livewire\Component;
use Livewire\WithPagination;

#[Layout('components.layouts.admin.app', ['title' => 'Tes Kompetensi Teknis'])]
class Index extends Component
{
    use WithPagination;

    public $jabatan_diuji;
    public $tgl_mulai;
    public $selected_id;

    public function render()
    {
        $data = Event::withCount(['peserta', 'ujianKompetensiTeknis'])
            ->with(['peserta', 'ujianKompetensiTeknis'])
            ->whereIsFinished('false')
            ->where('metode_tes_id', 4)
            ->orderByDesc('id')
            ->paginate(10);

        return view('livewire.admin.data-tes.tes-kompetensi-teknis.tes-berlangsung.index', compact('data'));
    }
}
